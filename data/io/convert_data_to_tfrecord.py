...
def convert_DOTAdataset_to_tfrecord():
    save_path = "/media/pinink/TOSHIBA_EXT/DOTA_train.tfrecord"
    mkdir('../tfrecord')
    writer = tf.python_io.TFRecordWriter(path=save_path)
    with open('/home/pinink/Desktop/tensorflow-yolov3/data/train_data/train_dataset.txt', 'r') as f:
        for line in f.readlines():
            ...
            boxes_num = len(annotation[1:]) // 5
            boxes = np.zeros([boxes_num, 5], dtype=np.float32)
            for i in range(boxes_num):
                boxes[i] = annotation[1 + i * 5:6 + i * 5]
            print("img_name:", image_name)
            print("img_height:", img_height)
            print("img_width:", img_width)
            print("num_objects:", boxes_num)
            print("boxes:", boxes)
            ...
            feature = tf.train.Features(feature={
                ...
                'gtboxes_and_label': _bytes_feature(boxes.tostring()),
                'num_objects': _int64_feature(boxes_num)
            })
            example = tf.train.Example(features=feature)
...
if __name__ == '__main__':
    ...
    convert_DOTAdataset_to_tfrecord()
