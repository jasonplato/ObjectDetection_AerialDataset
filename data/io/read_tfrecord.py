...
def read_single_example_and_decode(filename_queue):
    ...
    gtboxes_and_label = tf.cast(gtboxes_and_label,tf.int32)
...
def next_batch(dataset_name, batch_size, shortside_len, is_training):
    ...
    if dataset_name not in ['ship', 'spacenet', 'pascal', 'coco', 'DOTA']:
    ...
    if is_training:
        ...
        pattern = "/media/pinink/TOSHIBA_EXT/DOTA_train.tfrecord"
    else:
        # pattern = os.path.join('../data/tfrecord', dataset_name + '_test*')
        pattern = "/media/pinink/TOSHIBA_EXT/DOTA_test.tfrecord"
