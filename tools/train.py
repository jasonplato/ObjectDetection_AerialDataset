...
def train():
    ...
    lr = tf.train.exponential_decay(cfgs.LR, global_step, decay_steps=cfgs.DECAY_STEPS, decay_rate=cfgs.DECAY_RATE,
                                               staircase=True)
    ...
    #                                  values=[cfgs.LR, cfgs.LR / 10., cfgs.LR / 100.,cfgs.LR/1000,cfgs.LR/10000])
    ...
    with tf.Session(config=config) as sess:
        ...
        if restorer is not None:
            ...
            print('restore model')
        # print("image:", )
        # print("boxes:", gtboxes_and_label.shape)
        # print("objects:", num_objects.shape)
        ...
        for step in range(cfgs.MAX_ITERATION):
            ...
            if step % cfgs.SHOW_TRAIN_INFO_INTE != 0 and step % cfgs.SMRY_ITER != 0:
                ...
                print("no train info and no summary!")

            else:
                if step % cfgs.SHOW_TRAIN_INFO_INTE == 0 and step % cfgs.SMRY_ITER != 0:
                    start = time.time()

                    _, global_stepnp, img_name, boxes,num_object,rpnLocLoss, rpnClsLoss, rpnTotalLoss, \
                    fastrcnnLocLoss, fastrcnnClsLoss, fastrcnnTotalLoss, totalLoss = \
                        sess.run(
                            [train_op, global_step, img_name_batch, gtboxes_and_label_batch,num_objects_batch,rpn_location_loss, rpn_cls_loss, rpn_total_loss,
